...
from .utils import cal_mean, is_positive_semidefinite, kl_divergence
...
class NANO:
    ...
    def update_init(self, y, x_prior, P_prior):
        ...
        P_posterior_inv = hessian(lambda x: self.map_loss(x_prior, P_prior, x, y))(
            x_hat_posterior
        )
    ...
    def update(self, y):
        ...
        if self.init_type == "prior":
            ...
            x_hat, P_inv = self.update_iekf_init(
                y, x_hat_prior, self.P.copy(), self.iekf_max_iter
            )
        ...
        for _ in range(n_iterations):
            ...
            E_hessian = cal_mean(
                lambda x: self.jac_h(x).T @ np.linalg.inv(self.R)
            ...
            x_hat_next = x_hat - lr * (
                ...
                @ P_inv
                @ cal_mean(
                    lambda x: (x - x_hat) * self.loss_func(x, y), x_hat, P, self.points
                )
                + P_next @ P_inv_prior @ (x_hat - x_hat_prior)
            )
